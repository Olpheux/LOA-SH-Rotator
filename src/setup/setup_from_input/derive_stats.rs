...
fn calc_raid_captain_bonus(raid_captain_level: i64) -> f64 {
    if raid_captain_level > 0 {
        ...
        let move_speed: f64 = get_f64_input();

        if raid_captain_level == 3 { move_speed * 0.4 }
        else if raid_captain_level == 2 { move_speed * 0.22 }
        else if raid_captain_level == 1 { move_speed * 0.1 }
        else { 0.0 } // Shouldn't be possible to reach this, but stops 'potentially uninitilaized value' errors.
    } else { 0.0 }
}
...
fn calc_damage_modifier(equipment: &equipment_struct, spec: i64) -> f64 {
    let grudge_bonus = calc_grudge_bonus(equipment.engravings.grudge);
    let raid_captain_bonus = calc_raid_captain_bonus(equipment.engravings.raid_captain);
    ...
    let spec_bonus = spec as f64 * 0.000929825;
...
pub fn get(base_stats: &base_stats_struct, equipment: &equipment_struct) -> derived_stats_struct{
    derived_stats_struct{
        ...
        damage_modifier: calc_damage_modifier(equipment, base_stats.spec_stat),
